...
def _freeze_metadata(
    ...
    for k in sorted(metadata.keys()):
        ...
        if not isinstance(k, str) or not k:
            raise DocumentError(
                "Document.metadata keys must be non-empty str, "
                f"got {k!r}"
            )
        if not _IDENTIFIER_RE.fullmatch(k):
            raise DocumentError(
                "Document.metadata key must match "
                f"[A-Za-z_][A-Za-z0-9_.-]*, got {k!r}"
            )
        if len(k) > MAX_METADATA_KEY_LEN:
            raise DocumentError(
                "Document.metadata key length > "
                f"{MAX_METADATA_KEY_LEN}: {k!r}"
            )
        if not isinstance(v, str):
            raise DocumentError(
                f"Document.metadata[{k!r}] must be str, got "
                f"{type(v).__name__}"
            )
        if len(v) > MAX_METADATA_VALUE_LEN:
            raise DocumentError(
                f"Document.metadata[{k!r}] length > "
                f"{MAX_METADATA_VALUE_LEN}"
            )
        out[k] = v
    ...
    * ``metadata`` — key-sorted ``Mapping[str, str]``; serialisation
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.id, str) or not self.id:
            raise DocumentError(
                "Document.id must be non-empty str, got "
                f"{self.id!r}"
            )
        if not _IDENTIFIER_RE.fullmatch(self.id):
            raise DocumentError(
                "Document.id must match "
                f"[A-Za-z_][A-Za-z0-9_.-]*, got {self.id!r}"
            )
        if len(self.id) > MAX_DOCUMENT_ID_LEN:
            raise DocumentError(
                f"Document.id length > {MAX_DOCUMENT_ID_LEN}"
            )
        if not isinstance(self.content, str):
            raise DocumentError(
                "Document.content must be str, got "
                f"{type(self.content).__name__}"
            )
        if len(self.content) > MAX_DOCUMENT_CONTENT_LEN:
            raise DocumentError(
                f"Document.content length > "
                f"{MAX_DOCUMENT_CONTENT_LEN}"
            )
        if not isinstance(self.metadata, Mapping):
            raise DocumentError(
                "Document.metadata must be a Mapping, got "
                f"{type(self.metadata).__name__}"
            )
        if len(self.metadata) > MAX_METADATA_KEYS:
            raise DocumentError(
                f"Document.metadata count > {MAX_METADATA_KEYS}"
            )
...
class Query:
    ...
    * ``filter`` — metadata key/value equality filter (optional).
    ...
    text: str
    top_k: int = 5
    filter: Mapping[str, str] = dataclasses.field(
        default_factory=lambda: MappingProxyType({})
    )
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.text, str) or not self.text.strip():
            raise QueryError(
                "Query.text must be a non-empty str"
            )
        if len(self.text) > MAX_QUERY_LEN:
            raise QueryError(
                f"Query.text length > {MAX_QUERY_LEN}"
            )
        if not isinstance(self.top_k, int) or isinstance(
            self.top_k, bool
        ):
            raise QueryError(
                "Query.top_k must be int, got "
                f"{type(self.top_k).__name__}"
            )
        if self.top_k < MIN_TOP_K or self.top_k > MAX_TOP_K:
            raise QueryError(
                f"Query.top_k must be in [{MIN_TOP_K}, "
                f"{MAX_TOP_K}], got {self.top_k}"
            )
        ...
        if len(self.filter) > MAX_METADATA_KEYS:
            raise QueryError(
                f"Query.filter count > {MAX_METADATA_KEYS}"
            )
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.document, Document):
            raise KnowledgeStoreError(
                "RetrievedDocument.document must be Document, "
                f"got {type(self.document).__name__}"
            )
        if not isinstance(self.score, float):
            raise KnowledgeStoreError(
                "RetrievedDocument.score must be float, got "
                f"{type(self.score).__name__}"
            )
        if not math.isfinite(self.score) or self.score < 0.0:
            raise KnowledgeStoreError(
                "RetrievedDocument.score must be finite and "
                f">= 0.0, got {self.score!r}"
            )
        if not isinstance(self.rank, int) or isinstance(
            self.rank, bool
        ):
            raise KnowledgeStoreError(
                "RetrievedDocument.rank must be int, got "
                f"{type(self.rank).__name__}"
            )
        if self.rank < 0:
            raise KnowledgeStoreError(
                f"RetrievedDocument.rank must be >= 0, got "
                f"{self.rank}"
            )
    ...
    *why* a prompt was assembled: the original query, the ordered
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.query, Query):
            raise KnowledgeStoreError(
                "RetrievedContext.query must be Query, got "
                f"{type(self.query).__name__}"
            )
        if not isinstance(self.documents, tuple):
            raise KnowledgeStoreError(
                "RetrievedContext.documents must be tuple, got "
                f"{type(self.documents).__name__}"
            )
        for i, d in enumerate(self.documents):
            ...
            if d.rank != i:
                raise KnowledgeStoreError(
                    f"RetrievedContext.documents[{i}].rank must "
                    f"== {i}, got {d.rank}"
                )
        if not isinstance(self.digest, bytes):
            raise KnowledgeStoreError(
                "RetrievedContext.digest must be bytes, got "
                f"{type(self.digest).__name__}"
            )
        if len(self.digest) != 16:
            raise KnowledgeStoreError(
                "RetrievedContext.digest must be 16 bytes "
                f"(BLAKE2b-16), got {len(self.digest)}"
            )
    ...
    * ``{query}`` — replaced by the operator query text.
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.body, str) or not self.body.strip():
            raise TemplateError(
                "PromptTemplate.body must be a non-empty str"
            )
        ...
        if bad:
            raise TemplateError(
                "PromptTemplate.body contains unsupported "
                f"placeholders: {sorted(bad)!r}"
            )
...
class DocumentStoreTransport(Protocol):
    ...
    def read(
        self, doc_id: str
    ) -> Document | None:  # pragma: no cover - Protocol
        ...
...
class InMemoryDocumentStore:
    ...
    def read(self, doc_id: str) -> Document | None:
        if not isinstance(doc_id, str) or not doc_id:
            raise DocumentError(
                "InMemoryDocumentStore.read: doc_id must be "
                "non-empty str"
            )
...
class BM25Retriever:
    ...
    def __init__(self, *, transport: DocumentStoreTransport) -> None:
        if not isinstance(transport, DocumentStoreTransport):
            raise TransportError(
                "BM25Retriever.transport must implement "
                "DocumentStoreTransport"
            )
    ...
    def retrieve(self, *, query: Query) -> RetrievedContext:
        if not isinstance(query, Query):
            raise QueryError(
                "BM25Retriever.retrieve: query must be Query, "
        ...
        corpus: list[Document] = [
            d for d in self._transport.iter_documents()
            if _matches_filter(d, query.filter)
        ]
        n_docs = len(corpus)
        if n_docs == 0:
        ...
        for rank, (score, _id, idx) in enumerate(
            scored[: query.top_k]
        ):
            top.append(
                RetrievedDocument(
...
def _matches_filter(
    document: Document, filter_: Mapping[str, str]
) -> bool:
    if not filter_:
        return True
...
def _digest_context_inputs(
    query: Query, results: tuple[RetrievedDocument, ...]
) -> bytes:
    parts: list[str] = [
    ...
    for r in results:
        parts.append(
            f"r{r.rank}:id={r.document.id};"
            f"score={r.score:.12e};"
            f"content={r.document.content}"
        )
...
class PromptBuilder:
    ...
    def __init__(self, *, template: PromptTemplate) -> None:
        if not isinstance(template, PromptTemplate):
            raise TemplateError(
                "PromptBuilder.template must be PromptTemplate, "
    ...
    def build(self, *, context: RetrievedContext) -> Prompt:
        ...
        ctx_block = _CONTEXT_SEPARATOR.join(
            r.document.content for r in context.documents
        )
        ...
        try:
            ...
            raise TemplateError(
                f"PromptBuilder.build: render failed: {exc}"
            ) from exc
        return Prompt(
...
class KnowledgeStore:
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(transport, DocumentStoreTransport):
            raise TransportError(
                "KnowledgeStore.transport must implement "
                "DocumentStoreTransport"
            )
        if not isinstance(retriever, BM25Retriever):
            raise KnowledgeStoreError(
                "KnowledgeStore.retriever must be BM25Retriever"
            )
        if not isinstance(builder, PromptBuilder):
            raise KnowledgeStoreError(
                "KnowledgeStore.builder must be PromptBuilder"
            )
        self._transport = transport
