...
class CoinbaseAdapter(BaseAdapter):
    """
    Exchange adapter for coinbase.
    All calls are logged to the event ledger.
    """
    name = "coinbase"
    category = "CEX"
    trading_forms = frozenset({"SPOT", "PERP", "FUTURES"})
    ...
    def place_order(self, symbol: str, side: str, size: float,
        ...
        append_event("MARKET", "ORDER_PLACED", "coinbase", result)
        return result

    def cancel_order(self, order_id: str) -> bool:
        # TODO: implement live order cancellation
        append_event("MARKET", "ORDER_CANCELLED", "coinbase",
                     {"order_id": order_id})
        return True
