...
def test_event_types_have_kind_discriminator():
    assert SignalEvent(ts_ns=0, symbol="X", side=Side.BUY, confidence=0.5).kind is EventKind.SIGNAL
    assert (
        ExecutionEvent(
            ts_ns=0,
            symbol="X",
...
def test_engine_tier_registry_consistency():
    ...
    actual = {eng().name: eng().tier for eng in (*RUNTIME_ENGINE_CLASSES, *OFFLINE_ENGINE_CLASSES)}
