"""C-33 — tests for the Tianshou sandbox surface.

Mirrors the test-shape of :mod:`tests.test_elegantrl_sandbox` (C-32):

* value-object validation (frozen + slotted, finite-only floats,
  bounded ints);
* a deterministic in-memory :class:`TianshouPolicyTrainer` fake (the
  Protocol seam — no tianshou / torch / numpy / gymnasium imports
  happen at any point in this file);
* end-to-end :meth:`TianshouSandbox.train` walk producing a
  governance-shaped :class:`PatchProposal`;
* INV-15 3-run byte-identical replay;
* AST guards pinning the OFFLINE_ONLY tier (no tianshou / torch /
...
from evolution_engine.sandbox_tianshou import (
    MAX_MAX_EPOCH,
    MAX_PROPOSAL_ID_LEN,
    MAX_STEP_PER_EPOCH,
    MIN_MAX_EPOCH,
...
def test_module_advertises_new_pip_dependencies() -> None:
    assert NEW_PIP_DEPENDENCIES == ("tianshou", "gymnasium", "torch")


def test_proposal_source_is_canonical_module_path() -> None:
    assert PROPOSAL_SOURCE == "evolution_engine.sandbox_tianshou"


def test_step_per_epoch_bounds() -> None:
    assert MIN_STEP_PER_EPOCH == 1
    assert MAX_STEP_PER_EPOCH == 10_000_000


def test_max_epoch_bounds() -> None:
    assert MIN_MAX_EPOCH == 1
    assert MAX_MAX_EPOCH == 1_000
...
def test_policy_kind_enum_values_match_tianshou_class_names() -> None:
    assert TianshouPolicyKind.PPO.value == "PPOPolicy"
    assert TianshouPolicyKind.SAC.value == "SACPolicy"
    assert TianshouPolicyKind.TD3.value == "TD3Policy"
    assert TianshouPolicyKind.DDPG.value == "DDPGPolicy"
    assert TianshouPolicyKind.DQN.value == "DQNPolicy"


def test_policy_kind_count_is_five() -> None:
    assert len(list(TianshouPolicyKind)) == 5


# ---------------------------------------------------------------------------
# TianshouArguments validation
# ---------------------------------------------------------------------------


def _valid_args(**overrides: Any) -> TianshouArguments:
    base: dict[str, Any] = {
    ...
    return TianshouArguments(**base)


def test_arguments_constructs_with_defaults() -> None:
    args = _valid_args()
    assert args.policy_kind is TianshouPolicyKind.PPO
    assert args.random_seed == 0
    assert args.step_per_epoch == 512
...
def test_arguments_rejects_non_enum_policy_kind() -> None:
    with pytest.raises(TypeError):
        TianshouArguments(  # type: ignore[arg-type]
            policy_kind="PPO",
            random_seed=0,
        )
...
def test_arguments_rejects_below_min_max_epoch() -> None:
    with pytest.raises(ValueError):
        _valid_args(max_epoch=0)


def test_arguments_rejects_above_max_max_epoch() -> None:
    with pytest.raises(ValueError):
        _valid_args(max_epoch=MAX_MAX_EPOCH + 1)


def test_arguments_rejects_below_min_step_per_epoch() -> None:
    with pytest.raises(ValueError):
        _valid_args(step_per_epoch=0)


def test_arguments_rejects_above_max_step_per_epoch() -> None:
    with pytest.raises(ValueError):
        _valid_args(step_per_epoch=MAX_STEP_PER_EPOCH + 1)
...
def test_arguments_rejects_non_positive_step_per_collect() -> None:
    with pytest.raises(ValueError):
        _valid_args(step_per_collect=0)


def test_arguments_rejects_non_positive_repeat_per_collect() -> None:
    with pytest.raises(ValueError):
        _valid_args(repeat_per_collect=0)
...
# ---------------------------------------------------------------------------
# TianshouSandboxMetrics validation
# ---------------------------------------------------------------------------


def _valid_metrics(**overrides: Any) -> TianshouSandboxMetrics:
    base: dict[str, Any] = {
        "epochs_completed": 3,
        ...
        "final_critic_loss": 0.42,
        "final_policy_loss": 0.13,
    }
    base.update(overrides)
    return TianshouSandboxMetrics(**base)
...
def test_metrics_is_frozen_and_slotted() -> None:
    metrics = _valid_metrics()
    with pytest.raises(dataclasses.FrozenInstanceError):
        metrics.epochs_completed = 99  # type: ignore[misc]
    assert not hasattr(metrics, "__dict__")


def test_metrics_rejects_negative_epochs_completed() -> None:
    with pytest.raises(ValueError):
        _valid_metrics(epochs_completed=-1)
...
def test_metrics_rejects_inf_final_critic_loss() -> None:
    with pytest.raises(ValueError):
        _valid_metrics(final_critic_loss=float("inf"))
...
# ---------------------------------------------------------------------------
# TianshouSandboxResult validation
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Deterministic fakes for TianshouPolicyTrainer + MarketDynamics
# ---------------------------------------------------------------------------
...
class _FakeTianshouTrainer:
    """Deterministic :class:`TianshouPolicyTrainer` — returns canned metrics."""

    __slots__ = ("_metrics",)

    def __init__(self, *, metrics: TianshouSandboxMetrics) -> None:
...
# ---------------------------------------------------------------------------
# TianshouSandbox.train end-to-end
# ---------------------------------------------------------------------------


def _sandbox_inputs() -> tuple[
    _FakeDynamics, TianshouArguments, EpisodeConfig, _FakeTianshouTrainer
]:
    dynamics = _FakeDynamics()
    arguments = _valid_args()
    episode_config = EpisodeConfig(initial_notional_usd=1000.0, max_steps=8)
    trainer = _FakeTianshouTrainer(metrics=_valid_metrics())
...
def test_sandbox_constructs_with_valid_trainer() -> None:
    ...
    sandbox = TianshouSandbox(trainer=trainer)
...
def test_sandbox_rejects_non_trainer() -> None:
    with pytest.raises(TypeError):
        TianshouSandbox(trainer="not a trainer")  # type: ignore[arg-type]
...
def test_train_proposal_touchpoints_include_module_and_weights() -> None:
    ...
    assert "evolution_engine.sandbox_tianshou" in result.proposal.touchpoints
    assert "policy_weights" in result.proposal.touchpoints


def test_train_rationale_includes_policy_kind_and_digest() -> None:
    dynamics, arguments, episode_config, trainer = _sandbox_inputs()
    sandbox = TianshouSandbox(trainer=trainer)
    result = sandbox.train(
        dynamics=dynamics,
    ...
    assert "PPOPolicy" in result.proposal.rationale
    assert result.policy_digest in result.proposal.rationale


def test_train_proposal_meta_includes_digest_and_policy_kind() -> None:
    dynamics, arguments, episode_config, trainer = _sandbox_inputs()
    sandbox = TianshouSandbox(trainer=trainer)
    result = sandbox.train(
        dynamics=dynamics,
    ...
    assert result.proposal.meta["policy_kind"] == "PPOPolicy"
...
def test_train_proposal_meta_overlays_do_not_override_provenance() -> None:
    ...
    # Provenance wins: the canonical digest is used (not "ZZZZ").
    assert result.proposal.meta["policy_digest"] == result.policy_digest
    # User overlays still pass through.
...
def test_train_rejects_negative_ts_ns() -> None:
    dynamics, arguments, episode_config, trainer = _sandbox_inputs()
    sandbox = TianshouSandbox(trainer=trainer)
    with pytest.raises(TianshouSandboxConfigError):
        sandbox.train(
            dynamics=dynamics,
            arguments=arguments,
            episode_config=episode_config,
...
def test_train_uses_null_callback_by_default() -> None:
    ...
    assert isinstance(result, TianshouSandboxResult)
...
def test_train_rejects_trainer_returning_wrong_type() -> None:
    ...
    sandbox = TianshouSandbox(trainer=_BadTrainer())
    with pytest.raises(TypeError):
        sandbox.train(
            dynamics=dynamics,
            arguments=arguments,
...
def _run_once() -> TianshouSandboxResult:
    dynamics, arguments, episode_config, trainer = _sandbox_inputs()
    sandbox = TianshouSandbox(trainer=trainer)
...
def test_inv15_digest_changes_when_policy_kind_changes() -> None:
    dynamics, _, episode_config, trainer = _sandbox_inputs()
    sandbox = TianshouSandbox(trainer=trainer)
    r_ppo = sandbox.train(
        dynamics=dynamics,
        arguments=_valid_args(policy_kind=TianshouPolicyKind.PPO),
        episode_config=episode_config,
        ts_ns=1,
        proposal_id="p",
    )
    r_sac = sandbox.train(
        dynamics=dynamics,
        arguments=_valid_args(policy_kind=TianshouPolicyKind.SAC),
        episode_config=episode_config,
        ts_ns=1,
        proposal_id="p",
    )
    assert r_ppo.policy_digest != r_sac.policy_digest
...
def test_inv15_digest_is_blake2b_16_hex() -> None:
    ...
    # Re-compute via stdlib to confirm the digest space matches.
...
def test_null_callback_satisfies_protocol() -> None:
    cb = null_tianshou_callback()
    assert isinstance(cb, TianshouSandboxCallback)
...
def test_null_callback_methods_return_none() -> None:
    ...
    metrics = _valid_metrics()
    assert cb.on_training_start(ts_ns=0, step_per_epoch=1) is None
    assert (
        cb.on_step(
            ts_ns=0,
            step_idx=0,
...
# ---------------------------------------------------------------------------
# Convenience factories raise when tianshou missing
# ---------------------------------------------------------------------------


def test_tianshou_ppo_trainer_raises_when_dep_missing() -> None:
    try:
        ...
        pytest.skip("tianshou installed — production seam smoke skipped")


def test_tianshou_sac_trainer_raises_when_dep_missing() -> None:
    try:
        importlib.import_module("tianshou")
    except ImportError:
        with pytest.raises(ImportError, match="tianshou"):
...
def test_tianshou_policy_trainer_raises_when_dep_missing() -> None:
    try:
        importlib.import_module("tianshou")
    except ImportError:
        with pytest.raises(ImportError, match="tianshou"):
            tianshou_policy_trainer(policy_kind=TianshouPolicyKind.PPO)
    else:
        pytest.skip("tianshou installed — production seam smoke skipped")


def test_artifact_aliases_are_callable_typed() -> None:
    # PolicyArtifact = bytes; PolicyArtifactSink takes that and returns None.
...
_MODULE_PATH = Path(__file__).resolve().parents[1] / "evolution_engine" / "sandbox_tianshou.py"
...
def test_no_top_level_tianshou_import() -> None:
    assert all(not name.startswith("tianshou") for name in _top_level_imports(_module_ast()))
...
def test_no_engine_cross_imports_in_code() -> None:
    """Strip docstrings + comments; ensure no engine cross-reference in code."""
    tree = _module_ast()
    ...
    for needle in (
        ...
        # Special-case: "ui." is too generic; covered by import test.
        assert needle not in blob, needle


def test_tianshou_import_only_inside_factory() -> None:
    """tianshou may only be imported inside tianshou_policy_trainer."""
    tree = _module_ast()
    for node in ast.walk(tree):
        if isinstance(node, (ast.Import, ast.ImportFrom)):
            ...
            for name in names:
                if name.startswith(("tianshou", "torch", "gymnasium")):
                    # Walk parents to verify enclosed in the factory.
                    parent = _find_enclosing_function(tree, node)
                    assert parent is not None, (
                        f"top-level {name} import — must be inside tianshou_policy_trainer factory"
                    )
                    assert parent.name == "tianshou_policy_trainer", (
                        f"{name} imported in {parent.name!r} — must be "
                        "inside tianshou_policy_trainer"
                    )
...
def test_module_reload_is_idempotent() -> None:
    import evolution_engine.sandbox_tianshou as mod1

    importlib.reload(mod1)
    import evolution_engine.sandbox_tianshou as mod2

    # Same enum, same constants — no global state to drift.
    assert mod1.PROPOSAL_SOURCE == mod2.PROPOSAL_SOURCE
    assert mod1.MAX_STEP_PER_EPOCH == mod2.MAX_STEP_PER_EPOCH
    assert mod1.TianshouPolicyKind.PPO is mod2.TianshouPolicyKind.PPO
