"""C-32 — tests for the ElegantRL sandbox surface.

Mirrors the test-shape of :mod:`tests.test_sandbox` (A-01.2):

* value-object validation (frozen + slotted, finite-only floats,
  bounded ints);
* a deterministic in-memory :class:`ElegantPolicyTrainer` fake (the
  Protocol seam — no ElegantRL / torch / numpy imports happen at any
  point in this file);
* end-to-end :meth:`ElegantSandbox.train` walk producing a
  governance-shaped :class:`PatchProposal`;
* INV-15 3-run byte-identical replay;
* AST guards pinning the OFFLINE_ONLY tier (no elegantrl / torch /
...
from evolution_engine.sandbox_elegant import (
    MAX_MAX_STEP,
    MAX_PROPOSAL_ID_LEN,
    MAX_TARGET_STEP,
    MIN_MAX_STEP,
...
def test_module_advertises_new_pip_dependencies() -> None:
    assert NEW_PIP_DEPENDENCIES == ("elegantrl", "gymnasium", "torch")


def test_proposal_source_is_canonical_module_path() -> None:
    assert PROPOSAL_SOURCE == "evolution_engine.sandbox_elegant"


def test_target_step_bounds() -> None:
    assert MIN_TARGET_STEP == 1
    assert MAX_TARGET_STEP == 10_000_000


def test_max_step_bounds() -> None:
    assert MIN_MAX_STEP == 1
    assert MAX_MAX_STEP == 1_000_000
...
def test_agent_kind_enum_values_match_elegantrl_class_names() -> None:
    assert ElegantAgentKind.PPO.value == "AgentPPO"
    assert ElegantAgentKind.SAC.value == "AgentSAC"
    assert ElegantAgentKind.TD3.value == "AgentTD3"
    assert ElegantAgentKind.DDPG.value == "AgentDDPG"
    assert ElegantAgentKind.DQN.value == "AgentDQN"


def test_agent_kind_count_is_five() -> None:
    assert len(list(ElegantAgentKind)) == 5


# ---------------------------------------------------------------------------
# ElegantArguments validation
# ---------------------------------------------------------------------------


def _valid_args(**overrides: Any) -> ElegantArguments:
    base: dict[str, Any] = {
    ...
    return ElegantArguments(**base)


def test_arguments_constructs_with_defaults() -> None:
    args = _valid_args()
    assert args.agent_kind is ElegantAgentKind.PPO
    assert args.random_seed == 0
    assert args.target_step == 1024
...
def test_arguments_rejects_non_enum_agent_kind() -> None:
    with pytest.raises(TypeError):
        ElegantArguments(  # type: ignore[arg-type]
            agent_kind="PPO",
            random_seed=0,
        )
...
def test_arguments_rejects_below_min_target_step() -> None:
    with pytest.raises(ValueError):
        _valid_args(target_step=0)


def test_arguments_rejects_above_max_target_step() -> None:
    with pytest.raises(ValueError):
        _valid_args(target_step=MAX_TARGET_STEP + 1)


def test_arguments_rejects_below_min_max_step() -> None:
    with pytest.raises(ValueError):
        _valid_args(max_step=0)


def test_arguments_rejects_above_max_max_step() -> None:
    with pytest.raises(ValueError):
        _valid_args(max_step=MAX_MAX_STEP + 1)
...
def test_arguments_rejects_non_positive_repeat_times() -> None:
    with pytest.raises(ValueError):
        _valid_args(repeat_times=0)
...
# ---------------------------------------------------------------------------
# ElegantSandboxMetrics validation
# ---------------------------------------------------------------------------


def _valid_metrics(**overrides: Any) -> ElegantSandboxMetrics:
    base: dict[str, Any] = {
    ...
    base.update(overrides)
    return ElegantSandboxMetrics(**base)
...
def test_metrics_is_frozen_and_slotted() -> None:
    metrics = _valid_metrics()
    with pytest.raises(dataclasses.FrozenInstanceError):
        metrics.episodes_completed = 99  # type: ignore[misc]
    assert not hasattr(metrics, "__dict__")


def test_metrics_rejects_negative_episodes_completed() -> None:
    with pytest.raises(ValueError):
        _valid_metrics(episodes_completed=-1)
...
# ---------------------------------------------------------------------------
# ElegantSandboxResult validation
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Deterministic fakes for ElegantPolicyTrainer + MarketDynamics
# ---------------------------------------------------------------------------
...
class _FakeElegantTrainer:
    """Deterministic :class:`ElegantPolicyTrainer` — returns canned metrics."""

    __slots__ = ("_metrics",)

    def __init__(self, *, metrics: ElegantSandboxMetrics) -> None:
...
# ---------------------------------------------------------------------------
# ElegantSandbox.train end-to-end
# ---------------------------------------------------------------------------
...
def test_sandbox_rejects_non_trainer() -> None:
    with pytest.raises(TypeError):
        ElegantSandbox(trainer="not a trainer")  # type: ignore[arg-type]
...
def test_train_proposal_touchpoints_include_module_and_weights() -> None:
    ...
    assert "evolution_engine.sandbox_elegant" in result.proposal.touchpoints
    assert "policy_weights" in result.proposal.touchpoints


def test_train_rationale_includes_agent_kind_and_digest() -> None:
    dynamics, arguments, episode_config, trainer = _sandbox_inputs()
    sandbox = ElegantSandbox(trainer=trainer)
    result = sandbox.train(
        dynamics=dynamics,
    ...
    assert "AgentPPO" in result.proposal.rationale
    assert result.policy_digest in result.proposal.rationale


def test_train_proposal_meta_includes_digest_and_agent_kind() -> None:
    dynamics, arguments, episode_config, trainer = _sandbox_inputs()
    sandbox = ElegantSandbox(trainer=trainer)
    result = sandbox.train(
        dynamics=dynamics,
    ...
    assert result.proposal.meta["agent_kind"] == "AgentPPO"
...
def test_train_rejects_negative_ts_ns() -> None:
    dynamics, arguments, episode_config, trainer = _sandbox_inputs()
    sandbox = ElegantSandbox(trainer=trainer)
    with pytest.raises(ElegantSandboxConfigError):
        sandbox.train(
            dynamics=dynamics,
            arguments=arguments,
            episode_config=episode_config,
...
def test_train_uses_null_callback_by_default() -> None:
    ...
    assert isinstance(result, ElegantSandboxResult)
...
def test_train_rejects_trainer_returning_wrong_type() -> None:
    ...
    sandbox = ElegantSandbox(trainer=_BadTrainer())
    with pytest.raises(TypeError):
        sandbox.train(
            dynamics=dynamics,
            arguments=arguments,
...
def _run_once() -> ElegantSandboxResult:
    dynamics, arguments, episode_config, trainer = _sandbox_inputs()
    sandbox = ElegantSandbox(trainer=trainer)
...
def test_inv15_digest_changes_when_agent_kind_changes() -> None:
    dynamics, _, episode_config, trainer = _sandbox_inputs()
    sandbox = ElegantSandbox(trainer=trainer)
    r_ppo = sandbox.train(
        dynamics=dynamics,
        arguments=_valid_args(agent_kind=ElegantAgentKind.PPO),
        episode_config=episode_config,
        ts_ns=1,
        proposal_id="p",
    )
    r_sac = sandbox.train(
        dynamics=dynamics,
        arguments=_valid_args(agent_kind=ElegantAgentKind.SAC),
        episode_config=episode_config,
        ts_ns=1,
        proposal_id="p",
    )
...
def test_null_callback_satisfies_protocol() -> None:
    cb = null_elegant_callback()
    assert isinstance(cb, ElegantSandboxCallback)
...
def test_null_callback_methods_return_none() -> None:
    ...
    metrics = _valid_metrics()
    assert cb.on_training_start(ts_ns=0, target_step=1) is None
    assert (
        cb.on_step(
            ts_ns=0,
            step_idx=0,
...
# ---------------------------------------------------------------------------
# Convenience factories raise when ElegantRL missing
# ---------------------------------------------------------------------------


def test_elegantrl_ppo_trainer_raises_when_dep_missing() -> None:
    try:
        importlib.import_module("elegantrl")
    except ImportError:
        with pytest.raises(ImportError, match="elegantrl"):
            elegantrl_ppo_trainer()
    else:
        pytest.skip("elegantrl installed — production seam smoke skipped")


def test_elegantrl_sac_trainer_raises_when_dep_missing() -> None:
    try:
        importlib.import_module("elegantrl")
    except ImportError:
        with pytest.raises(ImportError, match="elegantrl"):
...
def test_elegantrl_agent_trainer_raises_when_dep_missing() -> None:
    try:
        importlib.import_module("elegantrl")
    except ImportError:
        with pytest.raises(ImportError, match="elegantrl"):
            elegantrl_agent_trainer(agent_kind=ElegantAgentKind.PPO)
    else:
        pytest.skip("elegantrl installed — production seam smoke skipped")
...
def test_elegantrl_import_only_inside_factory() -> None:
    """ElegantRL may only be imported inside elegantrl_agent_trainer."""
    tree = _module_ast()
    for node in ast.walk(tree):
            ...
            for name in names:
                if name.startswith(("elegantrl", "torch", "gymnasium")):
                    ...
                    assert parent.name == "elegantrl_agent_trainer", (
                        ...
                        "inside elegantrl_agent_trainer"
                    )
...
def test_module_reload_is_idempotent() -> None:
    import evolution_engine.sandbox_elegant as mod1

    importlib.reload(mod1)
    import evolution_engine.sandbox_elegant as mod2

    # Same enum, same constants — no global state to drift.
    assert mod1.PROPOSAL_SOURCE == mod2.PROPOSAL_SOURCE
    assert mod1.MAX_TARGET_STEP == mod2.MAX_TARGET_STEP
    assert mod1.ElegantAgentKind.PPO is mod2.ElegantAgentKind.PPO
