"""C-40 — tests for the pyro SVI/MCMC inference analyser surface."""
...
from intelligence_engine.svi_pyro import (
    ANALYSIS_SOURCE,
    MAX_ANALYSIS_ID_LEN,
...
def test_module_advertises_new_pip_dependencies() -> None:
    assert NEW_PIP_DEPENDENCIES == ("pyro-ppl", "torch", "numpy")


def test_analysis_source_is_canonical_module_path() -> None:
    assert ANALYSIS_SOURCE == "intelligence_engine.svi_pyro"
...
def test_inference_kind_values() -> None:
    assert SVIInferenceKind.SVI.value == "SVI"
    assert SVIInferenceKind.NUTS.value == "NUTS"
    assert SVIInferenceKind.HMC.value == "HMC"
    assert SVIInferenceKind.IMPORTANCE.value == "Importance"


def test_inference_kind_count() -> None:
    assert len(list(SVIInferenceKind)) == 4
...
def _valid_spec(**overrides: Any) -> SVIModelSpec:
    base: dict[str, Any] = {
        "num_sites": 2,
        "num_observations": 10,
...
def _valid_args(**overrides: Any) -> SVIInferenceArguments:
    base: dict[str, Any] = {
        "inference_kind": SVIInferenceKind.SVI,
        "random_seed": 42,
        "num_samples": 100,
    ...
    base.update(overrides)
    return SVIInferenceArguments(**base)


def test_args_constructs_with_defaults() -> None:
    a = _valid_args()
    assert a.inference_kind is SVIInferenceKind.SVI
...
def _valid_summary(**overrides: Any) -> SVISiteSummary:
    base: dict[str, Any] = {
        "name": "mu",
        "mean": 0.5,
        "std": 0.1,
...
def _valid_result(**overrides: Any) -> SVIInferenceResult:
    base: dict[str, Any] = {
        "site_summaries": (
    ...
    base.update(overrides)
    return SVIInferenceResult(**base)
...
def _valid_record(**overrides: Any) -> SVIInferenceRecord:
    base: dict[str, Any] = {
        "ts_ns": 1_000_000_000,
        "analysis_id": "aid-1",
    ...
    base.update(overrides)
    return SVIInferenceRecord(**base)
...
def test_record_rejects_non_spec() -> None:
    with pytest.raises(TypeError, match="spec must be SVIModelSpec"):
        _valid_record(spec="x")  # type: ignore[arg-type]


def test_record_rejects_non_result() -> None:
    with pytest.raises(TypeError, match="result must be SVIInferenceResult"):
...
def test_null_callback_satisfies_protocol() -> None:
    cb = null_svi_inference_callback()
    assert isinstance(cb, SVIInferenceCallback)
...
class _FakeEngine:
    """Deterministic SVI engine for testing."""
    ...
    def infer(
        ...
        *,
        ...
        # Deterministic per-site summary derived from seed + index.
        summaries = []
...
def test_analyser_emits_canonical_record() -> None:
    ...
    assert isinstance(record, SVIInferenceRecord)
...
def test_analyser_meta_contains_canonical_keys() -> None:
    ...
    assert record.meta["inference_kind"] == "SVI"
...
def test_analyser_rejects_non_args() -> None:
    a = PyroSVIAnalyser(engine=_FakeEngine())
    with pytest.raises(TypeError, match="arguments must be SVIInferenceArguments"):
        a.analyse(
            spec=_valid_spec(),
...
def test_analyser_rejects_long_analysis_id() -> None:
    a = PyroSVIAnalyser(engine=_FakeEngine())
    with pytest.raises(SVIAnalyserConfigError, match="analysis_id must be <="):
        a.analyse(
            spec=_valid_spec(),
            arguments=_valid_args(),
            ts_ns=0,
...
def test_analyser_rejects_engine_returning_wrong_type() -> None:
    ...
    a = PyroSVIAnalyser(engine=_BadEngine())  # type: ignore[arg-type]
    with pytest.raises(TypeError, match="must return SVIInferenceResult"):
        a.analyse(
            spec=_valid_spec(),
            arguments=_valid_args(),
            ts_ns=0,
...
def test_analyser_rejects_mismatched_site_count() -> None:
    ...
    a = PyroSVIAnalyser(engine=_MismatchEngine())  # type: ignore[arg-type]
    with pytest.raises(
        SVIAnalyserConfigError,
...
def test_analyser_threads_custom_callback() -> None:
    summaries_seen: list[SVISiteSummary] = []
    start_seen: list[SVIModelSpec] = []
    end_seen: list[SVIInferenceResult] = []
...
def test_inv15_digest_changes_with_kind() -> None:
    a = _run_once(kind=SVIInferenceKind.SVI)
    b = _run_once(kind=SVIInferenceKind.NUTS)
    assert a.analysis_digest != b.analysis_digest
...
def test_pyro_svi_engine_factory_signature() -> None:
    """Factory exists and can be called when packages installed.

    If pyro is not present, ImportError is raised with a helpful
    pip install hint that references NEW_PIP_DEPENDENCIES.
    """

    try:
        engine = pyro_svi_engine()
    except ImportError as exc:
        msg = str(exc)
        assert "pip install" in msg
        assert "pyro-ppl" in msg
        return
    assert isinstance(engine, SVIInferenceEngine)
...
def test_ast_no_top_level_pyro_import() -> None:
    assert "pyro" not in _top_level_imports()


def test_ast_no_top_level_torch_import() -> None:
    assert "torch" not in _top_level_imports()
...
def test_ast_vendor_imports_confined_to_factory() -> None:
    """All vendor imports must live inside ``pyro_svi_engine`` body."""
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and node.name == "pyro_svi_engine":
    ...
    assert "pyro" in factory_imports
    assert "torch" in factory_imports
    assert "numpy" in factory_imports
    assert {"pyro", "torch", "numpy"}.isdisjoint(other_imports)
...
def test_module_reload_is_idempotent() -> None:
    import intelligence_engine.svi_pyro as m1

    importlib.reload(m1)
    import intelligence_engine.svi_pyro as m2
