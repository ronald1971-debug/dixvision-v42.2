...
* AST guards: no top-level weaviate, no clock/random/os, no
  numpy/torch/polars, no engine cross-imports, no typed-event construction,
  ADAPTED FROM header present, weaviate confined to factory.
"""
...
from state.memory_tensor.semantic_weaviate import (
...
_THIS_MODULE = pathlib.Path("state/memory_tensor/semantic_weaviate.py").resolve()
...
def test_new_pip_dependencies():
    assert NEW_PIP_DEPENDENCIES == ("weaviate-client",)


def test_version():
    assert WEAVIATE_ADAPTER_VERSION == "1"


def test_distance_metric_values():
    assert DistanceMetric.COSINE.value == "cosine"
    assert DistanceMetric.DOT.value == "dot"
    assert DistanceMetric.L2_SQUARED.value == "l2-squared"
...
def test_store_construction_defaults():
    store = SemanticWeaviateStore(dim=4, max_size=10)
    assert store.dim == 4
    assert store.max_size == 10
    assert store.distance_metric is DistanceMetric.COSINE
    assert store.collection == "DIXMemoryEpisode"
    assert len(store) == 0


def test_store_rejects_non_int_dim():
    with pytest.raises(ValueError):
        SemanticWeaviateStore(dim=3.0, max_size=10)  # type: ignore[arg-type]


def test_store_rejects_zero_dim():
    with pytest.raises(ValueError):
        SemanticWeaviateStore(dim=0, max_size=10)


def test_store_rejects_negative_dim():
    with pytest.raises(ValueError):
        SemanticWeaviateStore(dim=-1, max_size=10)


def test_store_rejects_zero_max_size():
    with pytest.raises(ValueError):
        SemanticWeaviateStore(dim=4, max_size=0)


def test_store_rejects_non_metric():
    with pytest.raises(TypeError):
        SemanticWeaviateStore(dim=4, max_size=10, distance_metric="cosine")  # type: ignore[arg-type]


def test_store_rejects_empty_collection():
    with pytest.raises(ValueError):
        SemanticWeaviateStore(dim=4, max_size=10, collection="")
...
def test_store_satisfies_protocol():
    store = SemanticWeaviateStore(dim=2, max_size=4)
...
def test_evict_oldest_by_ts():
    store = SemanticWeaviateStore(dim=2, max_size=2)
...
# L2_SQUARED search
# ---------------------------------------------------------------------------
def test_search_l2_squared_zero_when_identical():
    store = SemanticWeaviateStore(dim=2, max_size=4, distance_metric=DistanceMetric.L2_SQUARED)
...
def test_search_l2_squared_pythagoras():
    """L2_SQUARED returns the squared distance — 25 for the (3, 4) point."""
    store = SemanticWeaviateStore(dim=2, max_size=4, distance_metric=DistanceMetric.L2_SQUARED)
    store.add(_ep("a", (0.0, 0.0)))
    q = MemoryQuery(ts_ns=100, query_id="q1", embedding=(3.0, 4.0), k=1)
    result = store.search(q)
    assert result.hits[0].distance == pytest.approx(25.0)
...
def test_search_dot_non_negative():
    store = SemanticWeaviateStore(dim=2, max_size=4, distance_metric=DistanceMetric.DOT)
    store.add(_ep("a", (1.0, 0.0)))
...
def test_serialize_independent_of_insert_order():
    s1 = SemanticWeaviateStore(dim=2, max_size=4)
    s1.add(_ep("a", (1.0, 0.0), ts=10))
    s1.add(_ep("b", (0.0, 1.0), ts=20))
    s2 = SemanticWeaviateStore(dim=2, max_size=4)
...
def test_deserialize_round_trip():
    ...
    restored = SemanticWeaviateStore.deserialize(blob)
...
def test_deserialize_rejects_non_bytes():
    with pytest.raises(TypeError):
        SemanticWeaviateStore.deserialize("not-bytes")  # type: ignore[arg-type]


def test_deserialize_rejects_corrupt():
    with pytest.raises(SemanticWeaviateError):
        SemanticWeaviateStore.deserialize(b"{not json")
...
def test_deserialize_rejects_wrong_version():
    ...
    with pytest.raises(SemanticWeaviateError):
        SemanticWeaviateStore.deserialize(blob)
...
def test_replay_byte_identical_three_runs():
    def run() -> bytes:
        store = SemanticWeaviateStore(dim=3, max_size=8)
...
def test_factory_rejects_empty_url():
    with pytest.raises(ValueError):
        weaviate_client_factory(url="")


def test_factory_rejects_non_str_api_key():
    with pytest.raises(TypeError):
        weaviate_client_factory(url=":embedded:", api_key=123)  # type: ignore[arg-type]
...
def test_factory_raises_when_dep_missing(monkeypatch):
    ...
    with pytest.raises(SemanticWeaviateError):
        weaviate_client_factory(url=":embedded:")
...
def test_no_top_level_weaviate_import():
    assert "weaviate" not in _top_level_imports(_parse())
...
def test_adapted_from_header_present():
    ...
    assert "# ADAPTED FROM: weaviate/weaviate-python-client" in text


def test_weaviate_import_confined_to_factory():
    tree = _parse()
    factory_fn: ast.FunctionDef | None = None
    for node in tree.body:
        if isinstance(node, ast.FunctionDef) and node.name == "weaviate_client_factory":
            factory_fn = node
            break
    ...
    assert "weaviate" in factory_imports
